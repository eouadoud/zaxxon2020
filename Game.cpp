	...
	, mIsMovingLeft(false)
{
	...
	_TextureWeapon.loadFromFile("Media/Textures/SI_WeaponJoueur.png");
	_TextureWeaponEnemy.loadFromFile("Media/Textures/SI_WeaponEnnemy.png");
...
void Game::HandleCollisionEnemyWeaponPlayer()
{
	for (std::shared_ptr<Entity> enemy : EntityManager::m_Entities)
	{
		...
		sf::FloatRect bndEnemy;
		bndEnemy = enemy->m_sprite.getGlobalBounds();

		sf::FloatRect bndPlayer;
		bndPlayer = EntityManager::GetPlayer()->m_sprite.getGlobalBounds();

		if (bndEnemy.intersects(bndPlayer) == true)
		{
...
void Game::HandleCollisionPlayerWall()
{
	// Handle collision weapon and walls
	...
	for (std::shared_ptr<Entity> player : EntityManager::m_Entities)
	{
		...
		for (std::shared_ptr<Entity> block : EntityManager::m_Entities)
		{
			...
			sf::FloatRect bndWalls;
			bndWalls = block->m_sprite.getGlobalBounds();

			if (boundPlayer.intersects(bndWalls) == true)
			{
